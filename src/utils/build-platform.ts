...
//“MAOXINHE_BUILD_PLATFORM”环境变量自定义命名构建平台
const BUILD_PLATFORM_OVERRIDE_KEY = "MAOXINHE_BUILD_PLATFORM";
...
export function detectBuildPlatform({
	...
	if (typeof overrideValue === "string" && overrideValue.trim() !== "") {
		// 环境变量显式覆盖最优先，可以用"MAOXINHE_BUILD_PLATFORM"环境变量自定义不同部署平台的名字（默认值为空，无定义，继续后续自动识别）
